...
def resynthesize(features, max_length=None):
    ...
    """
    TOTAL_LEN = (
        features.shape[-1] * HOP_SIZE
        if max_length == None
        else min(features.shape[-1] * HOP_SIZE, max_length)
    )
    omega = np.zeros_like(TOTAL_LEN)
    for i in range(features.shape[-1] - 1):
        last_omega = omega[i * HOP_SIZE - 1]
        omega[i * HOP_SIZE : (i + 1) * HOP_SIZE] = (
            np.arange(HOP_SIZE) / AUDIO_SR * 2 * np.pi * features[0, i] + last_omega
        )

    amplitudes = np.interp(
        x=np.linspace(0, 1, num=features.shape[-1] * HOP_SIZE),
