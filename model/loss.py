...
def nan_to_num(x, replacement = 0):
    '''
    Replaces all instances of nan with `replacement` in a tensor, in-place
    '''
...
class HumanReadableLoss(nn.Module):
    ...
    def forward(self, model_output, targets):
        ...
        f0_loss = F.mse_loss(output_f0, target_f0)
        voiced_loss = F.mse_loss(target_voiced, output_voiced)
        amp_loss = hps.amp_penalty * F.mse_loss(target_amp, output_amp)
     
        return f0_loss, voiced_loss, amp_loss
